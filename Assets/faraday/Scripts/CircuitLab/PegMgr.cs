...
public class PegMgr : MonoBehaviour, IPeg
{
    ...
    void Update () {
        ...
        if (original && !originalScript.IsHeld)
        {
            ...
            original.transform.parent = transform; //needs to be set before LockRotiation is called, that's why this might happen twice
            //Lock rotation to the nearest 90 degrees and update Position
            Point end = LockRotation(original, original);

            //Deal with physics and notify component
            PrepareComponentforBreadboard(originalScript,start, end);
...
public void PrepareComponentforBreadboard(CircuitComponent toPrepare, Point start, Point end)
    {
        // Lock it to the board
        toPrepare.transform.parent = transform;//need to make sure this happens even if this method is called from outside
        toPrepare.gameObject.GetComponent<Rigidbody>().useGravity = false;
        toPrepare.gameObject.GetComponent<Rigidbody>().isKinematic = true;
        ...
        toPrepare.gameObject.GetComponent<BoxCollider>().enabled = false;
        toPrepare.gameObject.GetComponent<SphereCollider>().enabled = false;

        // Notify the component itself where it has been placed
        Direction direction = GetDirection(start, end);
        toPrepare.Place(start, direction);
    }
    ...
    public Point LockRotation(GameObject target, GameObject reference)
    {
    ...
    public void UpdateOwnComponentTransformByDirection(CircuitComponent component, Direction direction){ //currently doenst work with long wire
        Vector3 newLocalRotation = new Vector3(-90,0,0);;
        Vector3 newLocalPosition = component.transform.localPosition;
        switch (direction){
            case Direction.North:
                newLocalRotation.z = 0;
                newLocalPosition.x = 0;
                newLocalPosition.z = -shortPositionOffset;
                break;
            case Direction.East:
                newLocalRotation.z = -90;
                newLocalPosition.x = shortPositionOffset;
                newLocalPosition.z = 0;
                break;
            case Direction.South:
                newLocalRotation.z = -180;
                newLocalPosition.x = 0;
                newLocalPosition.z = shortPositionOffset;
                break;
            case Direction.West:
                newLocalRotation.z = -270;
                newLocalPosition.x = -shortPositionOffset;
                newLocalPosition.z = 0;
                break;
        }
        component.transform.localEulerAngles = newLocalRotation;
