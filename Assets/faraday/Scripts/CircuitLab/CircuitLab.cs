...
public class CircuitLab : MonoBehaviour, ICircuitLab
{
            ...
            else if (battery.Generation == gen)
            {
                ...
                foreach (PlacedComponent component in components)
                {
                    ...
                    if (component.Generation == gen)
                    {
                        ...
                        component.VoltageEndOfComponentExport = new RealVoltageExport(op, isBattery ? component.End.ToString() : component.End.ToString());//Added for Voltmeter
                ...
                op.ExportSimulationData += (sender, args) =>
                {
                    ...
                    foreach (PlacedComponent component in components)
                    {
                        if (component.Generation == gen)
                        {
                            ...
                            //Update the voltage value at End of Component //Added
                            var voltageAtEnd = component.VoltageEndOfComponentExport.Value - minVoltage;
                            component.Component.SetEndOfComponentVoltage(voltageAtEnd);
    ...
    public PegMgr GetPegMgrFromBoard(Point pegCoords){
        GameObject pegObject = board.GetPeg(pegCoords).GameObject;
        return pegObject.GetComponent<PegMgr>();
    }
